...
## 追加実装：語彙改善とカード枚数増加（2026-02-24）

ユーザー体験の向上とシステムエラーの回避のため、初期計画からカード構成を大幅に刷新しました。

### 変更の目的
1. **文章構築エラーの回避**: 最大4人プレイ時の初期配布枚数がデッキ枚数を上回る数学的問題を解消。
2. **ビジネス構文化の強化**: 主語を内包する悪口カードを汎用フレーズ化し、助詞カードに「文末表現」を追加することで、滑らかで笑える文章が作れるように改善。

### 実装内容
- **語彙データの拡張**: `src/data/cards.json` を全170枚構成にアップデート。
  - Cushion: 30枚
  - Insult: 40枚 (状態・評価・比喩に特化)
  - Noun: 50枚 (具体的なビジネス対象)
  - Particle: 50枚 (助詞、接続副詞、文末表現)
- **設計書の更新**: `docs/game_design.md` の枚数・サンプル内容を同期。

### 検証項目
- [x] JSONパース・枚数バリデーション（全170枚）
- [x] 組み合わせ試行による文法破綻の解消確認
